...
pub const BETWEEN_EXCLUSIVE: [[Bitboard; 64]; 64] = unsafe {
    transmute(*include_bytes!("../embeds/between_exclusive.bin"))
};

// [square1][square2]
pub const LINE_THRU: [[Bitboard; 64]; 64] = unsafe {
    transmute(*include_bytes!("../embeds/line_thru.bin"))
};
...
pub const ZOBRIST_PIECES: [[[u64; 64]; 6]; 2] =
{
    ...
    while color < 2
    {
        ...
        while piece_type <= (super::types::PieceType::King as usize)
        {
...
mod tests
{
    ...
    fn test_between_exclusive()
    {
        for sq1 in Square::iter()
        {
            ...
            {
                assert!(BETWEEN_EXCLUSIVE[sq1][sq2] == BETWEEN_EXCLUSIVE[sq2][sq1]);

                if sq1 == sq2 { assert!(BETWEEN_EXCLUSIVE[sq1][sq2].is_empty()); }
            }
        }
        ...
        assert!(BETWEEN_EXCLUSIVE[Square::A2][Square::A4] == Bitboard::from(Square::A3));
        assert!(BETWEEN_EXCLUSIVE[Square::A2][Square::A8] == Bitboard::from(282578800148480));

        // Horizontal

        assert!(BETWEEN_EXCLUSIVE[Square::D4][Square::D5].is_empty());
        assert!(BETWEEN_EXCLUSIVE[Square::D4][Square::F5].is_empty());
        assert!(BETWEEN_EXCLUSIVE[Square::D4][Square::D6] == Bitboard::from(Square::D5));
        ...
        assert!(BETWEEN_EXCLUSIVE[Square::B1][Square::D3] == Bitboard::from(Square::C2));
        assert!(BETWEEN_EXCLUSIVE[Square::C2][Square::G6] == Bitboard::from(137707913216));
    }
    ...
    fn test_line_thru()
    {
        for sq1 in Square::iter()
        {
            for sq2 in Square::iter()
            {
                assert!(LINE_THRU[sq1][sq2] == LINE_THRU[sq2][sq1]);
                ...
                {
                    assert!(LINE_THRU[sq1][sq2] == Bitboard::from(sq1.file()));
                }
                else if sq1.rank() == sq2.rank()
                {
                    assert!(LINE_THRU[sq1][sq2] == Bitboard::from(sq1.rank()));
                }
            }
        }
        ...
        // Diagonal
        assert!(LINE_THRU[Square::B2][Square::C3] == Bitboard::from(9241421688590303745));
    }
