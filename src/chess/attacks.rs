...
impl<const ATTACKS_TABLE_SIZE: usize> MagicEntry<ATTACKS_TABLE_SIZE>
{
    pub fn attacks(&self, occ: Bitboard) -> Bitboard
    {
        ...
        debug_assert!((idx as usize) < ATTACKS_TABLE_SIZE, "Magic index out of bounds");
...
pub const PAWN_ATTACKS: [[Bitboard; 64]; 2] = unsafe {
    transmute(*include_bytes!("../embeds/pawn_attacks.bin"))
};

// [square]
pub const KNIGHT_ATTACKS: [Bitboard; 64] = unsafe {
    transmute(*include_bytes!("../embeds/knight_attacks.bin"))
};

// [square][key]
pub const BISHOP_ATTACKS: [MagicEntry<512>; 64] = unsafe {
    transmute(*include_bytes!("../embeds/bishop_attacks.bin"))
};

// [square][key]
pub const ROOK_ATTACKS: [MagicEntry<4096>; 64] = unsafe {
    transmute(*include_bytes!("../embeds/rook_attacks.bin"))
};

// [square]
pub const KING_ATTACKS: [Bitboard; 64] = unsafe {
    transmute(*include_bytes!("../embeds/king_attacks.bin"))
};
...
mod tests
{
    ...
    fn test_pawn_knight_king_attacks()
    {
        ...
        assert!(PAWN_ATTACKS[Color::Black][Square::A2] == Bitboard::from(Square::B1));

        assert!(KNIGHT_ATTACKS[Square::A3] == Bitboard::from(8657044482));
        assert!(KNIGHT_ATTACKS[Square::D4] == Bitboard::from(22136263676928));

        assert!(KING_ATTACKS[Square::B1] == Bitboard::from(1797));
        assert!(KING_ATTACKS[Square::D4] == Bitboard::from(120596463616));
    }
